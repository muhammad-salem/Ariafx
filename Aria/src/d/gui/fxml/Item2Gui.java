...
public class Item2Gui implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		TranslateTransition grapStart = new TranslateTransition(Duration.millis(500), stopHistoryGraph);
		...
		TranslateTransition grapStop = new TranslateTransition(Duration.millis(500), stopHistoryGraph);
		grapStop.setToX(0);
		grapStop.setToY(0);
	...
	public void linkStateProperty() {
		link.stateProperty().addListener((ob, old, newv)->{
			...
			case SCHEDULED:
			case CANCELLED:
				progress.setStyleClass(StyleProgress.WARNING);
				break;
			case RUNNING:
			...
			case SUCCEEDED:
				{
					if (link.getDownloaded() == link.getLength()) {			// in success
						progress.setStyleClass(StyleProgress.SUCCESS);
					}
					else if (link.getDownloaded() > link.getLength()) {		// in faild
						progress.setStyleClass(StyleProgress.DEFAULT);
					} 
					else {													// still in pause
						progress.setStyleClass(StyleProgress.WARNING);
					}
